...
ggplot(otu_table_filt_rare_removed_pct.mds.df, aes(x=NMDS1, y=NMDS2)) + stat_ellipse(alpha=0.8, aes(color=status), show.legend = F, lwd=0.2) + 
  ...
  annotate("text", x = (min(otu_table_filt_rare_removed_pct.mds.df$NMDS1) + max(otu_table_filt_rare_removed_pct.mds.df$NMDS1))/2, y = max(otu_table_filt_rare_removed_pct.mds.df$NMDS2)+0.1, 
...
RF_state_classify_sig <- rf.significance( x=RF_status_classify, xdata=otu_table_filt_rare_removed_pct,
...
RF_state_classify_loocv <- train(x=otu_table_filt_rare_removed_pct, y=metadata_filt$status,
